...
function getStickerLabel(pageIdx, slotIdx) {
  ...
  if (!page.special) {
    ...
    if (slotIdx === TEAM_SLOT) return `Time ${page.name}`;
...
function getStickerNumber(pageIdx, slotIdx) {
  ...
  if (!page) return '';
  if (!page.special) {
    if (slotIdx === SHIELD_SLOT) return '1';
    if (slotIdx === TEAM_SLOT) return '13';
    if (slotIdx < 13) return String(slotIdx);
